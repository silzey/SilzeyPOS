      ...
      <div className="overflow-x-auto whitespace-nowrap py-3 no-scrollbar flex gap-3 sm:gap-4 pl-4 pr-4">
        {products.map((product, index) => {
          ...
          return (
            <div
              key={product.id}
              className={`inline-flex flex-col items-center cursor-pointer group ${isFifthItem ? 'relative' : ''}`}
            ...
            >
              {isFifthItem && (
                <div className="absolute inset-[-3px] sm:inset-[-4px] rounded-full rotating-red-border z-0"></div>
              )}
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden border-2 border-primary/30 group-hover:border-primary shadow-md transition-all duration-200 relative transform group-hover:scale-105 z-10 bg-background">
                <Image
                  src={product.image}
                  alt={product.name}
                  layout="fill"
